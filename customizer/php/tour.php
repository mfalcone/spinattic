...
if ($action != 'getLastPosts') {
	...
	if ($id !=''){
		...
		$ssqlp = "SELECT tours.*, users.nickname ,DATE_FORMAT(date,'%d/%m/%Y') as fecha FROM tours, users where tours.iduser = users.id and tours.id = ".$id;
		...
		if($in_friendly != 1){ //Variable definida en friendlyURLmanager.php para ver si estoy llegando desde la friendly o no
			header("Location: http://".$_SERVER[HTTP_HOST]."/".$nickname."/".$friendly ,TRUE,301);
		}
...
//			window.prompt ("Copy to clipboard: Ctrl+C, Enter", text);
				...
				<?if($allow_embed == 'on'){?>
				  ...
				  <?if($allow_social == 'on'){?>
					...
					<div id="pano" class="content_tour">
						...
						<script>
							embedpano({swf:"http://<?php echo $_SERVER[HTTP_HOST];?>/player/tour.swf", xml:"http://<?php echo $_SERVER[HTTP_HOST];?>/customizer/data/xml.php?id=<?echo $id;?>", target:"pano", html5:"auto", wmode:"opaque", passQueryParameters:true});
						</script>                	
					</div>
				</div>
		...
		<div class="modulo_user">
					...
					<a href="http://<?php echo $_SERVER[HTTP_HOST];?>/profile.php?uid=<?php echo $iduser;?>" class="photo_user">
						<img width="100%" height="100%" src="http://<?php echo $_SERVER[HTTP_HOST];?>/images/users/<?echo $avatar;?>">
					</a>
					<div class="name_user">                    
						<p><a href="http://<?php echo $_SERVER[HTTP_HOST];?>/profile.php?uid=<?php echo $iduser;?>" class="link_name"><?echo $user;?></a></p>
						<?php get_pro();?>
					</div>
					<br clear="all">
...
<?
		$ssqlot = "SELECT tours.*, DATE_FORMAT(date,'%d/%m/%Y') as fecha, users.nickname FROM tours, users where tours.state = 'publish' and tours.privacy = '_public' and tours.id <> ".$id." and tours.iduser = ".$iduser." and tours.iduser = users.id ORDER BY id desc LIMIT 3";
...
?>
					...
					<div class="other">
						<a href="http://<?php echo $_SERVER[HTTP_HOST];?>/<?echo $rowot["nickname"];?>/<?echo $rowot["friendly_url"];?>"><p><?echo $rowot["title"];?></p></a>
						<a href="http://<?php echo $_SERVER[HTTP_HOST];?>/<?echo $rowot["nickname"];?>/<?echo $rowot["friendly_url"];?>" class="thumb"><img src="<?echo $rowot["tour_thumb_path"];?>thumb200x100.jpg"></a>
...
<?}?>
					<h3><a href="http://<?php echo $_SERVER[HTTP_HOST];?>/profile.php?uid=<?php echo $iduser;?>">View all</a></h3>
