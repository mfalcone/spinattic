...
<?php 
		$ssqlp = "SELECT blog.id, blog.title, blog.text, blog.image, categories_blog.category,categories_blog.id as category_id, DATE_FORMAT(blog.date,'%b %d %Y %h:%i %p') as fecha FROM blog, categories_blog where blog.category = categories_blog.id ".$filter." ORDER BY date desc";
		...
		while($row = mysql_fetch_array($result)){    
			...
			<article class="item notification" id="<?php echo $i?>">
				...
				<?php if($row["image"] != ''){?>
				<img src="images/blog/<?php echo $row["image"];?>" alt="alt text dummy">
				<?php }
				echo $row["text"];?>
