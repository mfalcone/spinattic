  ...
  public function handleUpload($uploadDir = null, $allowedExtensions = null) {
    ...
    if (!empty($this->allowedExtensions)) {
      if (!$this->checkExtension($this->fileExtension, $this->allowedExtensions)) {
        $this->setErrorMsg('Invalid file type (JPG only allowed)');
        return false;
      }
    }
...
function generate_image_thumbnail($source_image_path, $thumbnail_image_path, $force_height)
{
	...
	if($force_height == 1){
		...
		$thumbnail_image_width = $thumbnail_image_height *  $thumbnail_aspect_ratio;
...
function crop_image($source, $target, $thumb_width, $thumb_height){
