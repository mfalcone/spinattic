...
require_once("../inc/conex.inc");
    ...
    $ssqlp = "SELECT * FROM tours";
    $result = mysql_query($ssqlp);	
    while($row = mysql_fetch_array($result)){
    	echo $row["id"]."<br>";
    	make_tour_files($row["id"]);
    }
    echo "#ready";
...
function make_tour_files($idtour)
{
    ...
    if($row["state"]=='publish'){ //si esta en draft no genero el archivo
		...
		$friendlyuser = htmlspecialchars ($row["friendly_url"]);
	...
	/* PHP */
		...
		$final_data = '<?header("Location:http://'.$_SERVER[HTTP_HOST].'/'.$friendlyuser.'/'.$friendly_url.'?full");?>';
		
		/* REEMPLAZADO POR TOUR.PHP EN FULLSCREEN
		$final_data = '
			<?php 
			...
			<head>
				...
				<?php require_once(realpath($_SERVER["DOCUMENT_ROOT"])."/inc/fk-meta.inc");?>
			...
			<div id="pano" style="width:100%; height:100%;">
				...
				<script>
					embedpano({swf:"http://<?php echo $_SERVER[HTTP_HOST];?>/player/tour.swf", xml:"http://<?php echo $_SERVER[HTTP_HOST];?>/tours/'.$idtour.'/tour'.$version_xml.'.xml", target:"pano", wmode:"opaque", passQueryParameters:true});
				</script>
			</div>
	...
	*/
		/* Elimino todos los xml, html y php que haya */
    ...
    /*Envío al cloud    (no usamos mas cloud para tours)*/
    //Borro lo que haya
    //$salida = shell_exec('s3cmd -c /var/www/'.$bucket_config_file.' del s3://'.$cdn_string.'/tours/'.$idtour.' --recursive 2>&1');
    
    //Subo
    //$salida = shell_exec('s3cmd -c /var/www/'.$bucket_config_file.' put ' . $subfolder_add.'tours/'.$idtour.' s3://'.$cdn_string.'/tours/ --recursive 2>&1');
    
	//Borro local
    //$salida = shell_exec('rm -r ' . $subfolder_add.'tours/'.$idtour);
}
