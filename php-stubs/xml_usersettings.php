<?
	...
	if ($id !=''){
		...
		if($row = mysql_fetch_array($result)){
			...
			echo '<krpano><settings name="usersettings" username="'.htmlspecialchars($row["username"]).'" useravatar="http://'.$_SERVER[HTTP_HOST].'/images/users/'.$row["avatar"].'" userlink="[a href=http://'.$_SERVER[HTTP_HOST].'/profile.php?uid='.$id.' target=_blank]" userprofile="http://'.$_SERVER[HTTP_HOST].'/profile.php?uid='.$id.'"/></krpano>';
		}
	}
